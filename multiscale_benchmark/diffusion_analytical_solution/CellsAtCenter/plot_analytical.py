...
def main() -> None:
    ...
    parser.add_argument("--rate", type=float, default=1.6e-15,
                        help="Uptake rate [mol/min].")
    parser.add_argument("-T", "--tend", type=float,
                        default=7., help="End time [min].")
    ...
    c_0 = 10. * 1e-3  # [mol/m^3]
    L = 240. * 1e-6  # [m]
    rate = args.rate / 60.  # [mol/s]

    # Positions [m]
    x_central = np.array((0., 0., 0.))
    x_cells = np.array([[-10e-6, -10e-6, 0.],
                        [-10e-6, 10e-6, 0.],
                        [-10e-6, -30e-6, 0.],
                        [-10e-6, 30e-6, 0.],
                        [10e-6, -10e-6, 0.],
    ...
    sol = AnalyticalSolution(c_0=c_0, D=D, L=L, rate=rate,
                             x_cells=x_cells, count_modes=count_modes)
